...
local function last_file_for_dir(dir)
  local dir_path = normalize_dir(dir)
  local dir_prefix = dir_path:sub(-1) == "/" and dir_path or (dir_path .. "/")

  for _, path in ipairs(vim.v.oldfiles) do
    local normalized = vim.fs.normalize(path)

    if normalized:sub(1, #dir_prefix) == dir_prefix and is_file(normalized) then
      return normalized
    end
  end
end
...
api.nvim_create_autocmd({ "BufReadPost", "BufWinEnter" }, {
  group = api.nvim_create_augroup("restore_last_cursor", { clear = true }),
...
local function open_nvim_tree(data)
  ...
  local startup_target = last_file_for_dir(data.file)
  ...
  -- open the tree
  tree_state.restoring = true
  require("nvim-tree.api").tree.open({ path = data.file })

  vim.schedule(function()
    tree_state.restore(data.file)

    if startup_target and vim.api.nvim_win_is_valid(0) then
      vim.cmd("wincmd p")
      vim.cmd.edit(vim.fn.fnameescape(startup_target))
    end
  end)
end
