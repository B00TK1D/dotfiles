...
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(event_context)
    ...
    map("n", "<space>kn", vim.lsp.buf.rename, { desc = "varialbe rename" })
    map("n", "<space>ka", vim.lsp.buf.code_action, { desc = "LSP code action" })
...
for server_name, lsp_executable in pairs(enabled_lsp_servers) do
  if utils.executable(lsp_executable) then
    ...
    -- Skip missing servers silently to avoid startup hit-enter prompts.
  end
end
